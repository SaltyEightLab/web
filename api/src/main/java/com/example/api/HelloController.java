...
public class HelloController {
    ...
    @PostMapping("/seatarrangeBeta")
    public ResponseEntity<List<ForOutputStudent>> seatArrangeByRecievedDataBeta(@RequestBody DataWrapperBeta dataWrapperBeta) {
        try {
            // ①ClassRoom classRoom = new ClassRoom(5, 6);
            // クラスルームを生成する。
            System.out.println("1:classRoomを生成します。");
            int rows = dataWrapperBeta.getRows();
            int columns = dataWrapperBeta.getColumns();
            ClassRoom classRoom = new ClassRoom(rows, columns);
            ...
            // ③使用しない座席を設定する。
            System.out.println("3:使用しない座席を設定する。");
            List<Seat> isNotToBeUsed = dataWrapperBeta.getIsNotToBeUsed();
            for(Seat seat : isNotToBeUsed){
                classRoom.setToBeUsedForSeatAtPosition(seat.getFromFront(), seat.getFromRight(), false);
                System.out.println("from_front:" + seat.getFromFront() + "、from_right:" + seat.getFromRight() + "の座席を使用しない座席にしました。");
            }

            // ④【未実装】教卓に一番近い席を設定
            // classRoom.setSeatsNearTeacher(0, 4);
            System.out.println("4:教卓に一番近い席を設定します。");
            System.out.println("dataWrapperBeta.getSeatClosestTeacher():" + dataWrapperBeta.getSeatClosestTeacher());
            classRoom.setSeatsNearTeacher(dataWrapperBeta.getSeatClosestTeacher().getFromFront(), dataWrapperBeta.getSeatClosestTeacher().getFromRight());
            System.out.println("("+ dataWrapperBeta.getSeatClosestTeacher().getFromFront() + "," + dataWrapperBeta.getSeatClosestTeacher().getFromRight() + ")を教卓に一番近い席に設定しました。");
            ...
            // ⑥ 各生徒の条件を抽出
            System.out.println("6:各生徒の条件を抽出");
            List<Student> receivedStudentsInfo = dataWrapperBeta.getStudents();
            receivedStudentsInfo.sort(Comparator.comparingInt(Student::getIDforBackend));
            System.out.println("receivedStudentsInfo:" + receivedStudentsInfo);

            // ⑦public class ClassRoom
            // public Seat getSeatAtPosition(int row, int column)
            // public Student getCurrentStudent()によって生徒に対して個別の設定を行う。
            System.out.println("7-1:生徒に対して個別の希望条件の設定を行う。");
            // System.out.println(receivedStudentsInfo);
            int receivedStudentsInfoIndex = 0;
            System.out.println("AllStudents:");
            for(Seat seat : classRoom.getAllSeats()){
                System.out.println(seat.getCurrentStudent());
            }
            for (int j = 0; j < columns; j++) {
                for (int i = 0; i < rows; i++) {
                    Student student = classRoom.getSeatAtPosition(i, j).getCurrentStudent();
                    ...
                    Student receivedStudentInfo = receivedStudentsInfo.get(receivedStudentsInfoIndex);
                    receivedStudentsInfoIndex++;

                    // 希望する座席の条件を設定
                    student.setName(receivedStudentInfo.getName());
                    student.setIDforBackend(receivedStudentInfo.getIDforBackend());
                    student.setPrefersFrontRow(receivedStudentInfo.isPrefersFrontRow());
                    student.setPrefersFrontTwoRows(receivedStudentInfo.isPrefersFrontTwoRows());
                    student.setPrefersBackRow(receivedStudentInfo.isPrefersBackRow());
                    student.setPrefersBackTwoRows(receivedStudentInfo.isPrefersBackTwoRows());
                    ...
                    // for (Student studentToPlaceNextToInfo : studentsToPlaceNextToInfo) {
                    //     student.addStudentsToPlaceNextTo(students.stream()
                    //             .filter(s -> s.getIDforBackend() == studentToPlaceNextToInfo.getIDforBackend()).findFirst().orElse(null));
                    // }
                    // List<Student> studentsToPlaceWithinTwoSeatsInfo = receivedStudentInfo
                    //         .getStudentsToPlaceWithinTwoSeats();
                    // for (Student studentToPlaceWithinTwoSeatsInfo : studentsToPlaceWithinTwoSeatsInfo) {
                    //     student.addStudentsToPlaceWithinTwoSeats(
                    //             students.stream().filter(s -> s.getIDforBackend() == studentToPlaceWithinTwoSeatsInfo.getIDforBackend())
                    //                     .findFirst().orElse(null));
                    // }
                    // List<Student> studentsWantingAwayOneSeatFromMeInfo = receivedStudentInfo
                    //         .getStudentsWantingAwayOneSeatFromMe();
                    // for (Student studentWantingAwayOneSeatFromMeInfo : studentsWantingAwayOneSeatFromMeInfo) {
                    //     student.addStudentsWantingAwayOneSeatFromMe(
                    //             students.stream().filter(s -> s.getIDforBackend() == studentWantingAwayOneSeatFromMeInfo.getIDforBackend())
                    //                     .findFirst().orElse(null));
                    // }
                    // List<Student> studentsWantingAwayTwoSeatsFromMeInfo = receivedStudentInfo
                    //         .getStudentsWantingAwayTwoSeatsFromMe();
                    // for (Student studentWantingAwayTwoSeatsFromMeInfo : studentsWantingAwayTwoSeatsFromMeInfo) {
                    //     student.addStudentsWantingAwayTwoSeatsFromMe(
                    //             students.stream().filter(s -> s.getIDforBackend() == studentWantingAwayTwoSeatsFromMeInfo.getIDforBackend())
                    //                     .findFirst().orElse(null));
                    // }

                    System.out.println("rows:" + i + "、columuns:" + j + "、");
                    System.out.println(receivedStudentInfo
                    + "の情報を、");
                    System.out.println(student + "に入力しました。");
                }
            }

            System.out.println("7-2:生徒に対して個別の人間関係の設定を行う。");
            receivedStudentsInfoIndex = 0;
            for (int j = 0; j < columns; j++) {
                for (int i = 0; i < rows; i++) {
                    Student student = classRoom.getSeatAtPosition(i, j).getCurrentStudent();
           ...
           for(Seat seat : seats){
            ...
            // isCompleteSeatChangeMode)
            // 第２引数はpublic List<Student> getUnseatedStudents()
            boolean perfectSeatArrangeMode = dataWrapperBeta.getPerfectSeatArrangeMode();
            boolean fixedByGenderMode = dataWrapperBeta.getFixedByGenderMode();
            System.out.println("9:席替えを実行する。" + "perfectSeatArrangeMode:" + perfectSeatArrangeMode + "、" + "fixedByGenderMode:" + fixedByGenderMode);
            SeatArrangementEngine seatArrangementEngine = new SeatArrangementEngine();
            seatArrangementEngine.arrangeSeats(
                    classRoom,
            ...
            //お試し
            List<ForOutputStudent> studentsForOutput = classRoomManager.getStudentsForOutput();
            System.out.println("studentsForOutput:" + studentsForOutput);
    ...
    public static class DataWrapperBeta {
        private int rows;
        private int columns;
        private boolean perfectSeatArrangeMode;
        private boolean fixedByGenderMode;
        private List<Seat> isNotToBeUsed;
        private Seat seatClosestTeacher;
        private List<Student> students;
        ...
        public List<Seat> getIsNotToBeUsed() {
            return isNotToBeUsed;
        }

        public Seat getSeatClosestTeacher() {
            return seatClosestTeacher;
        }
        ...
        public void setIsNotToBeUsed(List<Seat> isNotToBeUsed) {
            this.isNotToBeUsed = isNotToBeUsed;
        }

        public void setSeatClosestTeacher(Seat seatClosestTeacher) {
            this.seatClosestTeacher = seatClosestTeacher;
        }
    }
}