  ...
  const culculateStudentsForDisplay = () => {
    const studentsForDisplay = students.map(student => ({
      ...
      studentsForDisplay.sort((a, b) => {
        const aValue = a.assignedSeat.from_front * layout.rows + -1 * (a.assignedSeat.from_right + 1 - layout.columns);
        const bValue = b.assignedSeat.from_front * layout.rows + -1 * (b.assignedSeat.from_right + 1 - layout.columns);
