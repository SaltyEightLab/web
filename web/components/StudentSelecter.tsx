...
const StudentSelecter: React.FC<StudentSelecterProps> = ({
  ...
  const addPairSelecter = () => {
    setPairSelecters([
      ...pairSelecters,
      { id: pairSelecters.length, student1: -1, student2: -1 },
    ]);
  };
  ...
  const handleSelectChange = (
    ...
    setPairSelecters(
      pairSelecters.map((pair) =>
        pair.id === id ? { ...pair, student1, student2 } : pair
      )
    );
  };
    ...
    if (students) {
      ...
      pairSelecters.forEach((pair) => {
        if (pair.student1 !== -1 && pair.student2 !== -1) {
          students[pair.student1].studentsToPlaceNextTo.push(
            students[pair.student2]
          );
        }
      });
    }
    ...
    >
      ...
      {pairSelecters.map((pair) => (
        <PairSelecter
          ...
          label={label}
          onSelect={(student1, student2) =>
            handleSelectChange(pair.id, student1, student2)
          }
        />
      ))}
      <button
